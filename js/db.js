...
/*
    القسم النشط حاليًا (mint / nursery)
    كل قسم له قاعدة بيانات منفصلة تمامًا
    داخل IndexedDB، فتبقى كل الحسابات
    والفواتير والتقارير مستقلة تمامًا
    بين القسمين.
*/
...
/*
    ...
    (يُستخدم فقط مع قسم "mint" الافتراضي
    لأنه القسم الوحيد الذي كان موجودًا
    قبل إضافة نظام الأقسام).
*/
...
/*
    مفتاح تخزين القسم النشط حاليًا
    داخل IndexedDB
*/

function getIndexedDbKey() {

    return `database_${currentWorkspace}`;

}


/*
    تعيين القسم النشط، وإعادة تهيئة
    حالة قاعدة البيانات المحلية حتى
    يُحمَّل قسم جديد بشكل نظيف عند
    الطلب التالي لـ openDatabase()
*/
...
/*
    ...
    المحلية (تُستخدم عند التبديل بين
    الأقسام أو الرجوع لشاشة الاختيار)
*/

function resetWorkspaceDatabaseState() {
...
function createDatabaseSchema() {
    ...
    sqliteDatabase.run(`
        ...
        CREATE INDEX IF NOT EXISTS
        idx_production_log_date
        ON production_log(date);
    ...
    sqliteDatabase.run(`
        ...
        CREATE TABLE IF NOT EXISTS waste_log (
            ...
            wasteType TEXT NOT NULL DEFAULT 'spoiled',
    ...
    /*
        ترقية الجداول القديمة بإضافة
        أعمدة جديدة دون فقدان أي بيانات
        موجودة (لأصحاب النسخ السابقة).
    */
    ...
    /*
        فهرس فريد جزئي لرقم الفاتورة
        (يتجاهل القيم الفارغة، حتى لا
        تتعارض السجلات القديمة قبل
        الترقيم بأثر رجعي أدناه)
    */
    ...
    /*
        ترقيم بأثر رجعي: أي عمليات بيع
        قديمة بدون رقم فاتورة (من قبل
        هذا التحديث) تُمنح رقمًا متسلسلًا
        الآن، بترتيب التاريخ.
    */
...
/*
    إضافة عمود لجدول موجود إن لم يكن
    موجودًا مسبقًا (ترقية آمنة لا تفقد
    أي بيانات)
*/
...
function ensureColumnExists(
    ...
    const info =
        sqliteDatabase.exec(
            `PRAGMA table_info(${safeTableName(table)})`
        );
    ...
    sqliteDatabase.run(
        `ALTER TABLE ${safeTableName(table)} ADD COLUMN ${column} ${definition}`
    );
...
/*
    منح أرقام فواتير متسلسلة لأي عمليات
    بيع قديمة لا تملك رقمًا بعد
    (تعمل مرة واحدة فعليًا، ثم لا تجد
    شيئًا لتفعله في المرات التالية)
*/

function backfillMissingInvoiceNumbers() {
...
/*
    نسخة متزامنة (غير async) لقراءة/كتابة
    إعداد، تُستخدم فقط أثناء إنشاء أو
    ترقية بنية قاعدة البيانات، قبل أن
    تكون بقية دوال القاعدة جاهزة للاستخدام
    العادي.
*/
